...
async function getWithdrawalRequests(searchParams: any) {
    let { searchText } = searchParams;
  
    let { page } = searchParams;
  
    const res = await fetch(
      `${SERVER_BASE_URL}/api/finance/withdrawals/service-provider/request?page=${page}&searchText=${searchText}`,
...
export default async function Page({ searchParams }: any) {
    const withdrawals = await getWithdrawalRequests(searchParams)
